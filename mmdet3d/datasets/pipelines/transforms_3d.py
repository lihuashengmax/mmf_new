...
from tools.combine_view_info import project_to_image
import torch 
...
class OurRandomFlip3D(object):
    ...
    def __call__(self, input_dict):
        ...
        if input_dict['pcd_vertical_flip']:
            ...
            input_dict['transformation_3d_flow'].extend(['VF'])
        #########更新一下lidar2img
        num_img = len(input_dict['cam_intrinsic'])
        assert num_img == len(input_dict['lidar2cam_r'])
        for i in range(num_img):
            viewpad = np.eye(4)
            viewpad[:3, :3] = input_dict['cam_intrinsic'][i]
            lidar2cam_rt = np.eye(4)
            lidar2cam_rt[:3, :3] = input_dict['lidar2cam_r'][i]
            lidar2cam_rt[:3, 3] = input_dict['lidar2cam_t'][i]
            lidar2img_rt = (viewpad @ lidar2cam_rt)
            input_dict['lidar2img'][i] = lidar2img_rt
        #########更新一下lidar2img
        ...
        # point_indices = points_in_boxes_cpu(input_dict['points'].tensor[:, :3], input_dict['gt_bboxes_3d'].tensor[:, :7])# N n_points
        # point_indices = torch.sum(point_indices, dim=0, keepdim=False).numpy()# n_points
        # mask = (point_indices != 0)
        # pointcloud = input_dict['points'].tensor[:, :3].numpy()[mask]
        # images = input_dict['img']
        # projection_matrices = input_dict['lidar2img']
        # base_filename = input_dict['sample_idx']
        # file_suffix = '_afterOurRandomFlip3D.png'
        # bbox_3d = input_dict['gt_bboxes_3d'].tensor[:, :7].numpy()
...
class ObjectSampleV2(object):
    ...
    def __init__(self, db_sampler, sample_2d=False, stop_epoch=None, update_view = False):
    ...
    def __call__(self, input_dict):
        ...
        if self.stop_epoch is not None and self.epoch >= self.stop_epoch:
            input_dict['gt_bboxes_isfusion'] = input_dict['gt_bboxes_isfusion'][:, :4]
            print('stop dbsampler')
            return input_dict
        
        # ##################db_sample之前的debug可视化
        # """
        # pointcloud (numpy.ndarray): 点云数据，形状为 (N, 3) 或 (N, 4)，其中每行是 (x, y, z, [intensity])。
        # images (list of numpy.ndarray): 包含六张图像的列表。
        # projection_matrices (list of numpy.ndarray): 包含六个投影矩阵的列表，每个矩阵形状为 (4, 4)。
        ...
        # from mmdet3d.ops import points_in_boxes_cpu
        # import torch
        # # pointcloud = input_dict['points'].tensor[:, :3].numpy()############点云可视化效果一般，尝试只可视化框内点云
        # point_indices = points_in_boxes_cpu(input_dict['points'].tensor[:, :3], input_dict['gt_bboxes_3d'].tensor[:, :7])# N n_points
        # point_indices = torch.sum(point_indices, dim=0, keepdim=False).numpy()# n_points
        # mask = (point_indices != 0)
        # pointcloud = input_dict['points'].tensor[:, :3].numpy()[mask]
        ...
        # projection_matrices = input_dict['lidar2img']
        # base_filename = input_dict['sample_idx']
        # file_suffix = '_beforedbsample.png'
        # bbox_3d = input_dict['gt_bboxes_3d'].tensor[:, :7].numpy()
        # project_pointcloud_to_images(pointcloud=pointcloud, images=images, projection_matrices=projection_matrices, base_filename=base_filename, file_suffix=file_suffix, bbox_3d = bbox_3d)
        # ##################db_sample之前的debug可视化
        ...
        if self.sample_2d:
            ...
            gt_bboxes_2d = input_dict['gt_bboxes_isfusion']
        ...
        if sampled_dict is not None:
            ...
            if self.sample_2d:
                ...
                input_dict['gt_bboxes_isfusion'] = gt_bboxes_2d
                input_dict['img'] = sampled_dict['img']

            if self.update_view:
                #同步view系列，与sparsefusion所用的pkl相对齐
                view_ids = []
                bboxes = []
                gt_names = []
                pts_centers = []
                ...
                img_h = input_dict['img_shape'][0]
                img_w = input_dict['img_shape'][1]
                sampled_gt_bboxes_3d_box = gt_bboxes_3d.new_box(sampled_gt_bboxes_3d)
                corners = sampled_gt_bboxes_3d_box.corners
                centers = sampled_gt_bboxes_3d_box.gravity_center
                dims = sampled_gt_bboxes_3d_box.dims
                yaws = sampled_gt_bboxes_3d_box.yaw[:,None]
                for ann_id in range(sampled_gt_bboxes_3d.shape[0]):#对于新增的每一个框
                    view_id = int(sampled_dict['gt_bboxes_2d'][ann_id][4])#找到这个新增的框所在的cam序号
                    viewpad = np.eye(4)
                    intrinsic = input_dict['cam_intrinsic'][view_id]
                    viewpad[:intrinsic.shape[0], :intrinsic.shape[1]] = intrinsic
                    lidar2cam_r = input_dict['lidar2cam_r'][view_id]
                    lidar2cam_t = input_dict['lidar2cam_t'][view_id]
                    lidar2cam_rt = np.eye(4)
                    lidar2cam_rt[:3, :3] = lidar2cam_r.T
                    lidar2cam_rt[3, :3] = lidar2cam_t#没有负号

                    centers_2d, center_cam_3d, view_mask = project_to_image(centers[ann_id:ann_id+1], viewpad, lidar2cam_rt.T, img_h, img_w)#多个
                    centers_2d = centers_2d[..., :3]#图片上的位置
                    center_cam_3d = center_cam_3d[..., :3]#仅经过外参处理

                    corners_view = corners[ann_id:ann_id+1].cpu().numpy()#多个
                    names_view = sampled_dict['gt_labels_3d'][ann_id:ann_id+1]#多个
                    centers_view = centers[ann_id:ann_id+1].cpu().numpy()#多个
                    centers_2d_view = centers_2d#已经多个
                    dims_view = dims[ann_id:ann_id+1].cpu().numpy()#多个
                    yaw_view = -yaws[ann_id:ann_id+1] - np.pi / 2#多个
                    center_cam_3d_view = center_cam_3d#已经多个
                    vel_3d_view = np.concatenate([sampled_dict['gt_bboxes_3d'][ann_id:ann_id+1][:, 7:9], np.zeros_like(1, 1)], axis=1)#多个

                    boxes_lidar_view = sampled_dict['gt_bboxes_3d'][ann_id:ann_id+1][:, 0:7]#多个
                    vels_lidar_view = sampled_dict['gt_bboxes_3d'][ann_id:ann_id+1][:, 7:9]#多个
                    ...
                    corner_2d, _, _ = project_to_image(corners_view[0], viewpad, lidar2cam_rt.T, img_h, img_w)  #一个
                    ...
                    bboxes.append([x1, y1, w, h])#8个角点取最大外接框作为2D的gt框
                    view_ids.append(view_id)
                    gt_names.append(names_view[0])
                    pts_centers.append(centers_view[0])
                    img_centers.append(centers_2d_view[0])
                    bboxes_cam_3d.append(bboxes_cam_3d_view[0])
                    vel_cam.append(vel_cam_view[0])
                    bboxes_lidar.append(boxes_lidar_view[0])
                    vels_lidar.append(vels_lidar_view[0])
                ...
                input_dict['gt_bboxes2d_view'] = np.concatenate([input_dict['gt_bboxes2d_view'], bboxes], axis = 0)#该视角下最大外接2D框
                input_dict['gt_names2d_view'] = input_dict['gt_names2d_view'] + gt_names#该视角下框的label
                input_dict['gt_viewsIDs'] = np.concatenate([input_dict['gt_viewsIDs'], view_ids], axis = 0)#该视角的视角序号
                input_dict['gt_pts_centers_view'] = np.concatenate([input_dict['gt_pts_centers_view'], pts_centers], axis = 0)#该视角的框在lidar坐标系下的中心点坐标
                input_dict['gt_img_centers_view'] = np.concatenate([input_dict['gt_img_centers_view'], img_centers], axis = 0)#该视角的框中心点通过内外参转换后的坐标，已经除过第三个数

                input_dict['gt_bboxes_cam_view'] = np.concatenate([input_dict['gt_bboxes_cam_view'], bboxes_cam_3d], axis = 0)#仅经过外参处理，在cam坐标系下的中心点、三维、yaw角
                input_dict['gt_velocity_cam_view'] = np.concatenate([input_dict['gt_velocity_cam_view'], vel_cam], axis = 0)#仅经过外参处理，在cam坐标系下的速度
                input_dict['gt_visible'] = np.concatenate([input_dict['gt_visible'], np.ones((sampled_gt_bboxes_3d.shape[0]), dtype=np.int32)], axis = 0)#该帧中所有框，1代表落在6张图片中，0代表在6张图片中不可见

                input_dict['gt_bboxes_lidar_view'] = np.concatenate([input_dict['gt_bboxes_lidar_view'], bboxes_lidar], axis = 0)#该视角的框在lidar坐标系下的中心点、三维、yaw角
                input_dict['gt_velocity_lidar_view'] = np.concatenate([input_dict['gt_velocity_lidar_view'], vels_lidar], axis = 0)#该视角的框在lidar坐标系下的速度

        if self.sample_2d:
            input_dict['gt_bboxes_isfusion'] = input_dict['gt_bboxes_isfusion'][:, :4]
            input_dict['gt_labels_isfusion'] = gt_labels_3d.astype(np.long)
        ...
        # projection_matrices = input_dict['lidar2img']
        # base_filename = input_dict['sample_idx']
        # file_suffix = '_afterdbsample.png'
        # bbox_3d = input_dict['gt_bboxes_3d'].tensor[:, :7].numpy()
        # project_pointcloud_to_images(pointcloud=pointcloud, images=images, projection_matrices=projection_matrices, base_filename=base_filename, file_suffix=file_suffix, bbox_3d = bbox_3d)
        # ##################db_sample之后的debug可视化
...
class OurGlobalRotScaleTrans(object):
    ...
    def __call__(self, input_dict):
        ...
        # projection_matrices = input_dict['lidar2img']
        # base_filename = input_dict['sample_idx']
        # file_suffix = '_afterOurGlobalRotScaleTrans.png'
        # bbox_3d = input_dict['gt_bboxes_3d'].tensor[:, :7].numpy()
        # project_pointcloud_to_images(pointcloud=pointcloud, images=images, projection_matrices=projection_matrices, base_filename=base_filename, file_suffix=file_suffix, bbox_3d = bbox_3d)
        # ##################db_sample之后的debug可视化
        return input_dict
