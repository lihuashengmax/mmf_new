...
input_modality = dict(
    ...
    use_map=False,
    use_external=False)
img_scale = (800, 448)#32的整数倍 200 112 100 56 50 28 25 14 13 7
num_views = 6
...
train_pipeline = [
    ...
    dict(type='MyLoadAnnotations3D', with_bbox_3d=True, with_label_3d=True, with_bbox=False, with_label=False, with_centers=False, with_cam_bbox=False, with_visible=False),
    ...
    dict(type='OurRandomAffine', scaling_ratio_range=(0.9, 1.1), flip_ratio=0.5, flip_sync_3d=True),#flip_sync_3d无用参数
...
freeze_lidar_components = False  # freeze the LiDAR backbone
freeze_lidar_detector = False  # freeze the LiDAR detector
freeze_img_backbone_neck = True

find_unused_parameters = False

runner = dict(type='CustomEpochBasedRunner', max_epochs=total_epochs)

load_from = 'pretrained/sparsefusion_R50_backbone_neck.pth'
# resume_from = 'work_dirs/mmf_r50_L_wimg_lcdb/epoch_15.pth'
